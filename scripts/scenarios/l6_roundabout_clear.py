"""
l6_roundabout_clear.py — Ego navigates a clear roundabout at low speed with no traffic
...
class L6RoundaboutClear(ScenarioBase):
    ...
    def run(self, ap=None, rec=None) -> dict:
        ...
        # No NPCs — clear roundabout navigation
        npcs = []

        # Enable autopilot on ego at low speed appropriate for roundabout
        if ap is None:
            ap = AutopilotController(self.ego, self.traffic_manager, target_speed_kmh=20)
        ap.target_speed_kmh = 20
        ap._configure_tm()
        ap.enable()
